        ...
        <div class="content pt-0 w-lg-50 w-100 mx-auto mt-2">
            <div class="d-flex align-items-center py-1 bg-white">
                ...
                <div class="text-success nav-link cat-tab3">
                    <a href="{{ route('training-page.create') }}" data-bs-toggle="tooltip"
                    ...
                    <div class="text-center" style="margin-left: 4.5rem;">
                        <h5 class="ms-1 mb-0" style="color: #247297;">Training Page</h5>
                    </div>
                </div>
                {{-- Add Details End --}}
            </div>
            <div>
                <table class="table table-bordered table-hover text-center">
                    ...
                    <tbody>
                        ...
                        @if (count($trainingPages)>0)
                            @foreach ($trainingPages as $key => $trainingPage)
                                <tr>
                                    <td class="text-center">{{ $loop->iteration }} </td>
                                    <td>{{ isset($words[$key]) ? Str::ucfirst($words[$key]) . ' ' . 'Tamplate' : 'Number out of range' }}
                                    </td>
                                    <td>
                                        <a href="{{ route('training-page.edit', [$trainingPage->id]) }}"
                                            class="text-primary">
                                            <i class="fa-solid fa-pen-to-square me-2 p-1 rounded-circle text-primary"></i>
                                        </a>
                                        <a href="{{ route('training-page.destroy', [$trainingPage->id]) }}"
